...
public class BluetoothLeService extends Service {
    ...
    private void updateValues(int rssi)
    {
        rssi = Math.abs(rssi);
        ...
        byte[] buf = new byte[]{(byte) 0x01, (byte) 0x00, (byte) 0x00, (byte) 0x00 , (byte) 0x00, (byte) 0x00 , (byte) 0x00  };
        buf[1] = (byte) Color.red(ColorToSet);
        buf[2] = (byte) Color.green(ColorToSet);
        buf[3] = (byte) Color.blue(ColorToSet);
        buf[4] = (byte)mGain;

        characteristicTx = getSupportedGattService()
                .getCharacteristic(BluetoothLeService.UUID_BLE_SHIELD_TX);
        characteristicTx.setValue(buf);
        writeCharacteristic(characteristicTx);

        Log.i(TAG , "from service: " + rssi );
    }
